...
function InstagramIcon({ active }: { active: boolean }) {
  return (
    <svg viewBox="0 0 24 24" className={`h-5 w-5 ${active ? 'text-pink-500' : 'text-slate-300'}`} fill="currentColor">
...
function ZaloIcon({ active }: { active: boolean }) {
  return (
    <svg viewBox="0 0 48 48" className={`h-5 w-5 ${active ? 'text-blue-500' : 'text-slate-300'}`} fill="currentColor">
...
function LinkedInIcon({ active }: { active: boolean }) {
  return (
    <svg viewBox="0 0 24 24" className={`h-5 w-5 ${active ? 'text-blue-700' : 'text-slate-300'}`} fill="currentColor">
        ...
        <div className="space-y-4 lg:col-span-1">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <dl className="space-y-2 text-sm">
              ...
              <div className="flex justify-between"><dt className="text-slate-500">Ngày tạo</dt><dd className="text-slate-700">{formatDate(customer.createdAt)}</dd></div>
            </dl>
          ...
          {(customer.leads?.length ?? 0) > 0 && (
            <div className="rounded-xl border border-slate-200 bg-white p-5">
              <h3 className="mb-3 font-semibold text-slate-900">Leads ({customer.leads!.length})</h3>
              <div className="space-y-2">
                {customer.leads!.map((l) => (
                  <Link key={l.id} href={`/leads/${l.id}`} className="flex items-center justify-between rounded-lg border border-slate-100 p-3 hover:bg-slate-50">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        <StatusBadge status={l.status} />
                        {l.product && (
                          <span className="text-xs text-slate-600">
                            Sản phẩm: <span className="font-medium text-slate-800">{l.product.name}</span>
                          </span>
                        )}
                      </div>
                      {(l.labels?.length ?? 0) > 0 && (
